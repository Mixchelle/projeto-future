...
class RecomendacaoListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        cliente = get_object_or_404(
            CustomUser, id=cliente_id, role__in=["cliente", "subcliente"]
        )

        # Verifica se o formulário existe e pertence ao cliente/subcliente
        formulario = get_object_or_404(
            FormularioRespondido, id=formulario_id, cliente_id=cliente_id
        )
        ...
        if self.request.user.role in ["funcionario", "gestor"]:
            ...
            self.request.user.role in ["cliente", "subcliente"]
            and self.request.user.id == cliente_id
        ):
            return Recomendacao.objects.filter(
                cliente_id=cliente_id, formulario_respondido_id=formulario_id
            )
            ...
            raise PermissionDenied(
                "Você não tem permissão para acessar estas recomendações."
            )
    ...
    def perform_create(self, serializer):
        ...
        if self.request.user.role not in ["funcionario", "gestor"]:
            raise PermissionDenied(
                "Apenas funcionários ou gestores podem criar recomendações."
            )
